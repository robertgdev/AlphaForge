...
class MacdCrossoverStrategy implements StrategyInterface
{
    ...
    public function configure(array $inputs): void
    {
        ...
        if (isset($inputs['signalPeriod'])) {
            $this->signalPeriod = (int) $inputs['signalPeriod'];
        }
    ...
    public function initialize(array $data): void
    {
        ...
        $minBars = max($this->slowPeriod + $this->signalPeriod, 50);
        $totalBars = $ohlcv->getTimestamps()->count();

        if ($totalBars < $minBars) {
            throw new \RuntimeException(
                sprintf(
                    'Insufficient data for MACD Crossover strategy. Need at least %d bars, got %d.',
                    $minBars,
                    $totalBars
                )
            );
