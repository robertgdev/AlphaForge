...
class BollingerBandReversalStrategy implements StrategyInterface
{
    ...
    private ?TimeSeriesInterface $middleBand = null;

    public function configure(array $inputs): void
    {
        if (isset($inputs['period'])) {
            $this->period = (int) $inputs['period'];
        }
        if (isset($inputs['stdDev'])) {
            $this->stdDev = (float) $inputs['stdDev'];
        }
    ...
    public function initialize(array $data): void
    {
        ...
        if ($totalBars < $this->period) {
            throw new \RuntimeException(
                sprintf(
                    'Insufficient data for Bollinger Band Reversal strategy. Need at least %d bars, got %d.',
                    $this->period,
                    $totalBars
                )
            );
        ...
        $this->middleBand = $bands->get('middle');

        $this->entryCondition = $close->crossesBelow($lowerBand);
        $this->exitCondition = $close->crossesAbove($this->middleBand);
