...
class OptimizeStrategyCommand extends Command
{
    protected $signature = 'alphaforge:optimize
        ...
        {--metric=sharpe_ratio : Metric to optimize (sharpe_ratio, total_return, win_rate, etc.)}';

    protected $description = 'Run strategy parameter optimization';

    public function handle(ParameterOptimizerService $optimizer, StrategyInputParser $inputParser): int
    {
        ...
        if ($useStrategyRanges) {
            ...
            $parameterRanges = $inputParser->parseInputs($paramsJson);
            if ($parameterRanges === false) {
        ...
        $combinations = $optimizer->generateCombinations($parameterRanges);
        ...
        $this->line("  Total combinations: $totalCombinations");
        $this->newLine();

        $optimizationRun = $optimizer->optimize(
