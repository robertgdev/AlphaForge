...
class DataUpdateCommand extends Command
{
    ...
    public function handle(
        ...
        if (! file_exists($filePath)) {
            if ($this->option('auto-generate') && ! $this->jsonEnabled()) {
                ...
                $genResult = $dataAutoGenerator->autoGenerate(
                    DataTypeConfig::fromOptions('ohlcv', null, null),
                    $exchange,
                    $market,
                    $timeframe,
                ...
                if (! $this->jsonEnabled()) {
                    $this->error("No market data file found for {$exchange}/{$market}/{$timeframe}.");
        ...
        try {
            ...
            return $this->outputJsonError("Failed to read data file: {$e->getMessage()}");
        }

        if ($header['numRecords'] === 0) {
            return $this->outputJsonError('Data file exists but contains no records. Use the import command instead.');
        }

        $lastRecord = $binaryStorage->readRecordByIndex($filePath, $header['numRecords'] - 1);

        if ($lastRecord === null) {
            return $this->outputJsonError('Could not read the last record from the data file.');
        }
        ...
        try {
            ...
            return $this->outputJsonError("Update failed: {$e->getMessage()}");
        } catch (\Throwable $e) {
            if (! $this->jsonEnabled()) {
                $this->finishProgressBarOnError();
            }
